...
.rag-OK {{ background: var(--green-l); color: var(--green); font-weight: 700; }}
...
<div class="tab-content" id="tab-help">
  ...
  <div class="info-box">
    ...
    <table>
      ...
      <tr><td><span style="background:var(--green-l);padding:2px 8px;border-radius:3px;font-weight:700;color:var(--green)">OK</span></td>
        <td class="left">Есть хотя бы один факт, но следующий этап ещё не запланирован</td>
        <td class="left">Работа по текущему этапу сделана. Ожидание плановой даты на следующий этап.</td></tr>
      <tr><td><span style="background:#F2F2F2;padding:2px 8px;border-radius:3px;font-weight:700;color:#888">WAIT</span></td>
        <td class="left">Плановые даты есть, но ни одного факта (план ≥ сегодня)</td>
        <td class="left">Ожидание: работа ещё не начата. Как только дата пройдёт без факта → AMBER/RED.</td></tr>
  ...
  <div class="info-box">
    ...
    <p><b>Прогресс ячейки</b> (на Dashboard) = средневзвешенное по подпродуктам внутри сегмента × инструмента. Веса: КК из справочника (Обычная 96%, РБ 1%, Пастила 0.5%, Секьюритизация 2.5%), остальные продукты — равные доли (1/N подпродуктов).</p>
    <p><b>Общий прогресс</b> (KPI-карточка) = среднее арифметическое по всем ячейкам Dashboard. Каждая ячейка (сегмент × инструмент) считается равной.</p>
  </div>
...
function renderKPIs() {{
  ...
  document.getElementById('kpis').innerHTML = `
    ...
    <div class="kpi green"><div class="val">${{rows.filter(r => r.rag === 'OK').length}}</div><div class="lbl">OK <span class="info-tip" title="Текущий этап выполнен, следующий ещё не запланирован">ⓘ</span></div></div>
    <div class="kpi"><div class="val">${{rows.filter(r => r.rag === 'WAIT').length}}</div><div class="lbl">WAIT <span class="info-tip" title="Ожидание: план в будущем, но работа ещё не начата (нет фактов)">ⓘ</span></div></div>`;
}}
