  ...
  <div class="info-box">
    ...
    <table>
      ...
      <tr><td><span style="background:var(--green-l);padding:2px 8px;border-radius:3px;font-weight:700;color:var(--green)">OK</span></td>
        ...
        <td class="left">По графику: работа идёт, сроки не нарушены.</td></tr>
...
function renderKPIs() {{
  ...
  document.getElementById('kpis').innerHTML = `
    ...
    <div class="kpi green"><div class="val">${{rows.filter(r => r.rag === 'OK').length}}</div><div class="lbl">OK <span class="info-tip" title="По графику: работа идёт, ближайший план ещё не наступил">ⓘ</span></div></div>
  ...
  segs.forEach(seg => {{
    ...
    instrs.forEach(instr => {{
      ...
      if (!active.length) {{
        ...
        const pAvg = active.reduce((s, r) => s + r.weight * r.progress, 0) / tw;
        const pMax = Math.max(...active.map(r => r.progress || 0));
        const started = active.filter(r => (r.progress || 0) > 0).length;
        const total = active.length;
        const pct = Math.round(pAvg * 100);
        if (pAvg >= 1.0) {{
          h += `<td class="mx-done">100%</td>`;
        }} else if (pMax > 0) {{
          h += `<td class="mx-prog">${{started}}/${{total}}<br><span style="font-size:9px;opacity:.7">${{Math.round(pMax*100)}}% макс</span></td>`;
        }} else {{
